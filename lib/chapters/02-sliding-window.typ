...
This is a case of dynamic sliding window! We can start at the first character, then grow the window right until a character repeats. When that happens we shrink the window from the left until the repetitions is eliminated and we continue. The answer is the longetst window we ever had at any point so we'll keep a running best score. Let's see it in pseudo code:
